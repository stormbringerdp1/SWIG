...
//#include <QStaticText>
  ...
  #include <QStatusBar>
//============================================================================================
MPlot::MPlot(QWidget* pParent, const bool isReport, const char* cYAxisLabel, float minYNormalValue, float maxYNormalValue)
...
//      connect(PScrollBar, SIGNAL(sliderMoved(int)), this, SLOT(SliderMoved(int)));
...
//      PScrollBar->setPageStep(gxdel*NMaxVisibleCount+0.5);
  ...
  if(fyMaxValue < MaxYNormalValue)  fyMaxValue = MaxYNormalValue;
  if(MinYNormalValue < fyMinValue)  fyMinValue = MinYNormalValue;

  qreal dfYMax = fyMaxValue - MaxYNormalValue;
  qreal dfYMin = MinYNormalValue - fyMinValue;
...
//  if(fyMinValue <= MaxYNormalValue && MaxYNormalValue <= fyMaxValue) {
  qreal gyMax = gy0Top + ky*(fyMaxValue-MaxYNormalValue);
...
//  if(fyMinValue <= MinYNormalValue && MinYNormalValue <= fyMaxValue) {
  qreal gyMin = gy0Top + ky*(fyMaxValue-MinYNormalValue);
//    painter.setPen(penRange); painter.drawLine(QPointF(gx0Left, gy), QPointF(gx0Right, gy));
...
//====  Draw Curve:  ====
  ...
  j = PosSB/dgx;
  delLeft = (-PosSB/dgx + j)*dgx;
  for(int i = NItemVisibleLeft; i < qMin(NItemVisibleLeft+NMaxVisibleCount+1, count); i++) {
    gx = gx0Left + delLeft + (i-NItemVisibleLeft)*dgx;
    fp = (*pListValues)[i];

    if(fp.y() < MinYNormalValue || MaxYNormalValue < fp.y())  penPoint.setBrush(QColor(200,0,60));
    else                                                      penPoint.setBrush(QColor(0,200,60));
